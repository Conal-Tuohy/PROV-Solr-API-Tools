...
-->
	...
	<xsl:variable name="collection-id">
		<!--
		 ...
		 access to the actual query URL which was used in its invocation, we will generate the URL 
		 with the parameters sorted into a canonical order: first the "q" parameter, then all the others,
